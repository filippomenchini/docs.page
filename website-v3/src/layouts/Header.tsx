...
export default function Header() {
  const { source, baseBranch, owner, repository, config } = useContext();
        ...
        <div className="mx-4 flex h-16 items-center px-4 lg:mx-0 lg:px-8">
          ...
          >
            {!!config.logo && (
              <img
                alt="Logo"
                className={cx('hidden h-[30px]', { 'dark:inline-block': !config.logoDark })}
                src={getImagePath(source, baseBranch, config.logo)}
              />
            )}
            {!!config.logoDark && (
              <img
                ...
                src={getImagePath(source, baseBranch, config.logoDark)}
              />
            )}
